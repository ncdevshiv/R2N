...
| M1 React compatibility — hooks, keys, context, effects | in progress | 15/18 |
| M2–M7 | planned | — |

**Overall: 51/106 roadmap tasks.** Every claim above is backed by the test suite (`cargo test`) and the architecture-guard tests. See [roadmap/CHECKLIST.md](roadmap/CHECKLIST.md) for the task-level record, [JOURNAL.md](JOURNAL.md) for decision history, and [docs/AUDIT.md](docs/AUDIT.md) for the latest audit report.
...
cargo test --workspace          # 149 tests incl. architecture + acceptance guards
