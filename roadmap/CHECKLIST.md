...
> Generated 2026-08-29 · updated 2026-09-03 · **65/108** tasks done (60%).
> Audit basis: every task was verified against the actual implementation and test suite (297 tests green, clippy clean, JS compatibility 120/132 = 91% of the test262-aligned subset — function identity fixed after the harness found `f === f` was false, CLI verified end-to-end on all examples AND on the real-world TodoMVC app: `r2n render app.jsx` renders the full header/main/footer tree and the real `todoReducer` executes ADD/TOGGLE/REMOVE correctly), not against earlier claims.
...
`IN PROGRESS` · weeks 12–20 · progress **11/17** (65%)
...
- [x] **P0** — Promises + async/await with scheduler-driven continuations: `new Promise(executor)` (sync executor, (resolve, reject) settlers, sync-throw rejects), `Promise.resolve/reject` statics, `.then(onOk, onErr)`/`.catch` (always-async jobs, chaining with pass-through identity, handler results incl. returned promises ADOPTING into the chained promise, settle idempotence, self-adoption TypeError); async arrows lower to a SEGMENT STATE MACHINE (`JsExpr::AsyncFn` + `JsAsyncSegment`): segment 0 sync per ECMA, each `await` suspends (bind target / `return await` completion), continuations are `EffectJob` scheduler jobs drained to fixpoint by `drain_jobs`; a segment error or rejected await rejects the result promise (async fns never throw synchronously); await restricted to statement positions (`let x = await p;` / `x = await p;` / `await p;` / `return await p;`) with precise compile errors elsewhere; setState inside continuations re-renders via the existing scheduler (tests/promises.rs, 17 tests)
- [x] **P1** — Generators & iterator protocol: top-level `function*` declarations lower to the T07 segment machine (yield = pull-based twin of await; same JsAsyncSegment); `Runtime` binds them in a new GLOBAL env that chains into every component env (Env::child_of now shares parent frames); generator instances run PULL-based (`g.next(arg)` injects arg as the yield expression's value incl. `let x = yield v` binds, `{value, done}` results, post-done `{undefined, true}` forever, `g.return(v)` early-completes, `g.throw(e)` raises at the caller and kills the instance, laziness = nothing runs before the first next); array iterators `.values()/.entries()/.keys()` over snapshots with the same `{value, done}` protocol; yield restricted to statement positions with precise compile errors elsewhere (tests/generators.rs, 13 tests)
- [x] **P0** — Modules — import/export/dynamic import: `import { a, b as c } from "p"` / `import Def from "p"` / `import * as ns from "p"` / side-effect `import "p"` and combinations (both parser twins), `export default Name` / `export { a, b as c }`, dynamic `import("path")` expression; multi-module LINKER (`crates/r2n-compiler/src/link.rs`): `ModuleResolver` trait (FsResolver relative-path + `.r2n` defaulting, lexically normalized — no `\\?\` prefix; MemResolver for tests/hosts), DFS module-graph discovery with cycle detection (precise `ImportCycle` path error), diamond dedup, PRE-order global component table (deterministic artifacts), export surfaces (components + generator decls; unknown export = precise link error), dynamic-import discovery by full AST walk + specifier canonicalization to resolved ids, dev/prod StrictMode strip; IR `ModuleIr` carries each module's export surface in the artifact; RUNTIME binds `@module:{id}` namespace Maps in the global env (`import("widget")` resolves to the SAME canonical namespace), `ComponentRefVal` renders in value/children position as a component mount, a component value in TAG position renders as a JSX tag (local binding, prop-passed, or namespace member `<m.Widget/>` — props and children flow), generator/function value imports documented as deferred; 19 tests (compiler/tests/link.rs 9: cycle, unknown export, no-default, alias, default, dynamic discovery, canonicalization, diamond dedup, cross-module render; runtime/tests/modules.rs 10: imported component render, dynamic-import namespace, dynamic-only module, ComponentRefVal in binding/prop/namespace positions, JSX tag + props/children) (tests/link.rs + tests/modules.rs, 19 tests)
- [ ] **P0** — Module initialization order (top-level statements, TDZ, side-effect ordering): top-level `let`/`const` executing in deterministic module-evaluation order with side effects sequenced along the dependency graph, TDZ-aware import bindings. NOT YET IMPLEMENTED — the current dialect has no top-level statements (modules contain only declarations), so the gap is latent; must be closed before top-level statement syntax is added
- [x] **P1** — Destructuring, spread, rest: object/array patterns in function bodies, component bodies, top-level `let`/`const`, and params (`const {a, b: c} = o`, `const [x, ...rest] = arr`, `Header({dispatch})`, `editing = false` defaults via `undefined`-guarded assigns); rest collects leftovers (`...rest` objects via `$rest` member builtin, arrays via `$restFrom`); general statement grammar in plain functions — `if`/`while`/`for` (update runs on `continue`, skipped on `break`, via `While.step`)/`switch` (runtime fall-through driver, `break` exits, `default` supported)/early `return` + bare `return;` (control-flow error channel: `Return(Value)` boxed in `RuntimeError`, caught at call/async-step/generator-step/callback/reducer/handler/memo-factory/effect boundaries); `break`/`continue` lower to runtime raises (total — try-block statements accept full grammar via `stmt_to_block_expr`); function expressions (`memo(function Item(){...})`); multi-interpolation templates (`` `${a} ${b}` `` — interpolation-close no longer advances through next_token); exported functions + `memo(fn)` consts link as COMPONENTS (React semantics — `<Header/>` resolves cross-module); entry root falls back to its sole component (`export function App()` needs no default); linker probes `.r2n`/`.js`/`.jsx`, skips external packages (`react`, `classnames`, `react-router-dom` → runtime builtins: `memo` identity, real `classnames`, `useLocation` → `{pathname: "/"}`, `useReducer` accepts module-level reducer values) and `.css` side-effect imports; engine binds component params BY PROP NAME (destructured params receive props directly); builtins `concat`/`every`/`Math.random` (xorshift64*); FIRST REAL APP: `r2n render app.jsx` on TodoMVC renders the full header/main/footer tree and the real `todoReducer` executes ADD/TOGGLE/REMOVE correctly (tests/statements.rs, 23 tests; compiler/tests/link.rs entry-fallback split into two tests)
- [ ] **P1** — Rest parameters (`...args` in params): precise `Unsupported` error today — needs call-site arg-vector support (FuncIr/Closure rest binding) (split from T10, T10b)
...
- [x] **P0** — test262-subset conformance harness + published compatibility score: upstream test262 files cannot run yet (they need `var`/loops/`switch`/`assert()`/template literals — the dialect has none), so the harness is test262-ALIGNED: 132 authored cases each pinning ONE ECMA-262 semantic in the dialect with its ECMA-section reference, observed behaviorally via exact `console.log` comparison after one flush (async drains to fixpoint). Honest scoring: ECMA-conformant cases are hard regression gates; 12 known gaps/divergences pin today's output, count as not-passing, and are listed in docs/COMPATIBILITY.md with reasons — a fix fails the consistency test until promoted. **Published score: 120/132 = 91%** (100% on values/==/===/closures/objects/exceptions/promises/generators/classes; gaps in `-`/`<` coercion, BigInt-mixing TypeError, ≥1e21/<1e-6 number formatting, string-index undefined); 14 category tests + `published_scorecard_matches_harness` consistency test + env-gated triage/scorecard dumps; the harness FOUND the function-identity bug (`f === f` was false — env-pointer comparison on a cloned Env) and the fix promoted it with two new identity cases (tests/test262_subset.rs, 16 tests; docs/COMPATIBILITY.md)
